...
public class Navigation extends App {
	...
	Navigation(Drone sp, ArrayList<Polygon> nfp, ArrayList<Sensor> s) {
		...
		final_path = new ArrayList<Point>();
	}
    	...
    	for(var i=0;i<neighbors.size();i++) {
    		...
    		if(!neighbor.getVisited() &&actual_pnt.clearFlightPath(li,no_fly_zones)) {
        		...
        		if(n_lbl>=0) {
        			...
        			var num=minn;
        			// case when we look to reach a sensor
        			if(closest_sensor<34) {
        				//if neighbor is in target sensor's area, return it
		        		if(n_lbl==closest_sensor) {
		        			
		        			return Pair.with(i*10, neighbor);
		        		}
		        		// if not, calculate distance to it
		        		num = neighbor.getDistance(sensors.get(closest_sensor-1).getP());
	        		}
        			else {
        				// case when we look to reach back the starting point
        				num = neighbor.getDistance(start_p.getP());
        			}
        			// save this state as the next one if distance is minimum
	        		if(num<minn) {
	            		minn = num;
	            		angle = i * 10;
	            		next_pnt = neighbor;
	...
	private void collectData(Drone actual_pnt, int closest_sensor) {
        ...
        while(made_moves<MAX_MOVES) {
        	...
        	if(label>=0) {
        		// if next state is on map:
        		
        		if(label == closest_sensor) {
        			// if next state is in target sensor's area:
        			sensors.get(label-1).setVisited(true);
        			count_sensors++;
        			closest_sensor = next_pnt.getClosestSensor(sensors).getLabel();
        		}
        		// transition to next state, save the angle at which drone travels
        		...
        		// if there is no next state accessible, change direction:
        		if(angle<180) angle_h = angle+180;
        		else angle_h = angle-180;
        	}
        	
        	if(closest_sensor==-2) {
        		// if drone has visited all sensors:
        		
        		path_added = true;
        		path.remove(path.size()-1);
        		final_path.addAll(path);
        		collectData(actual_pnt, 34);
        		break;
        	}
        	else
        	if(closest_sensor==34) {
        		// if drone is going back to starting point:
        		
        		if(actual_pnt.getDistance(start_p.getP())<RANGE_WITHIN_SENSOR) {
        			// if it's reached starting point's area
        			
        			path_added=true;
        			if(path.size()>1) final_path.addAll(path);
            		break;
            	}
        	}
        }
        // if number of maximum moves is reached and
        //		- we didn't visit all sensors:
        //		OR
        //		- we didn't reach the starting point's area:
        if(!path_added && path.size()>1) final_path.addAll(path);
	}
	...
	protected void main() {
        ...
        if(lbl>0) {
        	// if starting point is in the area of a sensor:
        	sensors.get(lbl-1).setVisited(true);
        	count_sensors++;
        }
        // calculate the closest sensor's label
        var closest_sensor = start_p.getClosestSensor(sensors).getLabel();
        // start the algorithm
        collectData(start_p, closest_sensor);
        // construct the solution for readings.geojson
        addLineToList(final_path);
	}
}
